...
import * as lexiconHelpers from "../lib/lexiconHelpers";
...
function generateDataSegment(
  ...
  return isFormatted ? (
    <span>
      <strong>{label}</strong>{" "}
      <span style={{ fontSize }}>{(text && getFormatted(text)) || ""}</span>
    </span>
  ) : (
    <span>
      <strong>{label}</strong> <span style={{ fontSize }}>{text}</span>
    </span>
  );
}
...
function generateWordPart(
  ...
  if (strongsNum) {
    return (
      <div key={key} style={{ margin: "0px 10px 0px 10px", maxWidth: "400px" }}>
        {generateLine(pos)}
    ...
    // not main word
    return (
      <div key={key} style={{ margin: "0px 0px 0px 10px", maxWidth: "400px" }}>
        {generateLine(pos)}
        {generateWordEntry(multipart, word, origLangFontSize)}
        {generateDataSegment(translate("morphology"), morphStr)}
        <br />
...
function movePrimaryWordToTop(partCount, wordParts) {
  ...
  indices.forEach((i) => {
    ...
    const partLen = ((wordParts && wordParts.length > i && wordParts[i]) || "")
      .length;
...
function getStrongsAndLexicon(strong, lexiconData, pos) {
  ...
  if (
    ...
    lexiconData[lexiconId][strongNumber]
  ) {
 ...
 * @return {*[]}
 */
export function generateWordLexiconDetails(
  wordObject,
  lexiconData,
  translate,
  generateWordPart,
  ...
  const partCount = Math.max(
    ...
    strongsParts.length,
    wordParts.length
  ); // since there may be inconsistancies, use largest count
  ...
  if (partCount < 2) {
    ...
    wordLexiconDetails = generateWordPart(
      translate,
      wordObject.lemma,
      morphStrs[0],
      strongNumber,
...
class WordLexiconDetails extends React.Component {
  render() {
    ...
    let wordLexiconDetails = generateWordLexiconDetails(
      wordObject,
      lexiconData,
      translate,
      generateWordPart,
